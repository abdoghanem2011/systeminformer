    ...
    case PH_POWERACTION_TYPE_FIRMWAREBOOT:
        {
            if (!PhGetOwnTokenAttributes().Elevated)
            {
                PhShowMessage2(
                    ...
                    L"Make sure SysInform is running with administrative privileges."
                    );
                break;
            }
    ...
    )
{
    ...
    if (PhGetIntegerSetting(SETTING_ENABLE_WARNINGS))
    {
        result = PhShowConfirmMessage(
            ...
            L"freeze",
            Process->ProcessName->Buffer,
            L"Freezing does not persist after exiting SysInform.",
            FALSE
            );
    }
    else
        ...
        if (!NT_SUCCESS(status))
        {
            ...
            else
            {
                ...
                PhShowStatus(
                    ...
                    L"Unable to unload ",
                    Module->Name->Buffer,
                    L". Make sure SysInform is running with "
