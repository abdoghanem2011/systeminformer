    ...
    )
{
    ...
    PhInitFormatS(&format[1], L"\\SysInform");
    PhInitFormatS(&format[2], L"\\SysInform");
    ...
    )
{
    ...
    config.pszMainInstruction = L"Initializing SysInform kernel driver...";
    ...
    )
{
    ...
    if (PhDoesOldKsiExist())
    {
        PhShowKsiMessageEx(
            ...
            FALSE,
            L"Unable to load kernel driver",
            L"The last SysInform update requires a reboot."
            );
        return;
    }
    ...
    )
{
    ...
    if (!NT_SUCCESS(status = PhHttpInitialize(&httpContext)))
        goto CleanupExit;
    if (!NT_SUCCESS(status = PhHttpConnect(httpContext, L"SysInform.io", PH_HTTP_DEFAULT_HTTPS_PORT)))
        goto CleanupExit;
