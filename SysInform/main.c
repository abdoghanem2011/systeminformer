    ...
    )
{
    ...
    if (!NT_SUCCESS(PhInitializePhLib(L"SysInform")))
        return 1;
    ...
    if (PhIsExecutingInWow64())
    {
        PhShowWarning2(
            ...
            L"Warning.",
            L"%s",
            L"You are attempting to run the 32-bit version of SysInform on 64-bit Windows. "
            L"Most features will not work correctly.\n\n"
            L"Please run the 64-bit version of SysInform instead."
            );
...
/**
 * The main message loop for SysInform.
 *
    ...
    )
{
    ...
    fileName = PhConcatStrings(5, PhGetString(directory), L"SysInform", L"_DumpFile_", alphastring, L".dmp");
...
/**
 * Unhandled exception filter callback for SysInform.
 *
    ...
    )
{
    ...
    if (NT_SUCCESS(PhIsInteractiveUserSession()))
    {
        ...
        config.pszMainInstruction = L"SysInform has crashed :(";
    ...
    else
    {
        ...
        title = PhCreateString(L"SysInform has crashed :(");
#ifdef DEBUG
        message = PhFormatString(
            L"%s\r\n0x%08X (%s)\r\n%s",
 ...
 * single-instance behavior and activate previous instances of SysInform.
...
/**
 * Initializes the execution policy for SysInform.
 *
 * This function checks if the Shift key is held down during startup. If so, it attempts
 * to launch Task Manager (`taskmgr.exe`) instead of SysInform.
 * This provides a quick way for users to access Task Manager if needed, for example,
 * if SysInform is set as the default Task Manager replacement and the user
 ...
 * to that desktop, and launches a new instance of SysInform on it. After the
 ...
 * throughout the SysInform application for timing operations and measurements.
 */
 ...
 * 
 * This function performs the necessary initialization of the SysInform
 ...
 * required for the SysInform application to function properly.
 */
    ...
    )
{
    ...
    if (!PhStartupParameters.NoSettings)
    {
        ...
        // 2. A file named SysInform.exe.settings.json in the program directory. (This changes
        ...
        if (PhIsNullOrEmptyString(PhSettingsFileName))
        {
            PhSettingsFileName = PhGetKnownLocationZ(PH_FOLDERID_RoamingAppData, L"\\SysInform\\settings.json", TRUE);
        }
        ...
        if (!PhIsNullOrEmptyString(PhSettingsFileName))
        {
            ...
            if (status == STATUS_FILE_CORRUPT_ERROR)
            {
                if (PhShowMessage2(
                    ...
                    TD_WARNING_ICON,
                    L"SysInform's settings file is corrupt. Do you want to reset it?",
    ...
    )
{
    ...
    else
    {
        ...
        if (upperValue)
        {
            ...
            {
                // User probably has SysInform replacing Task Manager. Force
 ...
 *
 * This function defines and processes all supported command line options for SysInform.
