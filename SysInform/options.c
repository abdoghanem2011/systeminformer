            ...
            case IDC_RESET:
                {
                    if (PhShowMessage2(
                        ...
                        TD_WARNING_ICON,
                        L"Do you want to reset all settings and restart SysInform?",
                        L""
                        ) == IDYES)
                    {
    ...
    )
{
    ...
    {
        static CONST PH_STRINGREF valueName = PH_STRINGREF_INIT(L"SysInform");
    ...
    )
{
    ...
    {
        message = L"Do you want to make SysInform the default Windows Task Manager?";
    ...
    )
{
    ...
    if (PhpIsDefaultTaskManager())
    {
        PhSetWindowText(GetDlgItem(WindowHandle, IDC_DEFSTATE), L"SysInform is the default Task Manager:");
        PhSetWindowText(GetDlgItem(WindowHandle, IDC_REPLACETASKMANAGER), L"Restore default...");
    }
    else
    {
        PhSetWindowText(GetDlgItem(WindowHandle, IDC_DEFSTATE), L"SysInform is not the default Task Manager:");
    ...
    )
{
    ...
    if (RestartRequired)
    {
        if (PhShowMessage2(
            ...
            TD_YES_BUTTON | TD_NO_BUTTON,
            TD_INFORMATION_ICON,
            L"One or more options you have changed requires a restart of SysInform.",
            L"Do you want to restart SysInform now?"
            ) == IDYES)
        {
            SysInform_PrepareForEarlyShutdown();
            ...
            if (programFilesPath = PhExpandEnvironmentStringsZ(L"%ProgramFiles%\\"))
            {
                if (!PhStartsWithString(applicationFileName, programFilesPath, TRUE))
                {
                    if (PhShowMessage2(
                        ...
                        TD_YES_BUTTON | TD_NO_BUTTON,
                        TD_WARNING_ICON,
                        L"WARNING: You have not installed SysInform into a secure location.",
                        L"%s is not recommended when running SysInform from outside a secure location (e.g. Program Files).\r\n\r\nAre you sure you want to continue?",
                        Message
                        ) == IDNO)
                    {
    ...
    )
{
    ...
    SysInform_Invoke(PhpOptionsNotifyChangeCallback, NULL);
}
            ...
            case IDC_FONT:
                {
                    ...
                    if (!GetCurrentFont(&font))
                    {
                        ...
                        GetObject(SysInform_GetFont(), sizeof(LOGFONT), &font);
                    }
...
{
    COLOR_ITEM(SETTING_COLOR_OWN_PROCESSES, L"Own processes", L"Processes running under the same user account as SysInform."),
